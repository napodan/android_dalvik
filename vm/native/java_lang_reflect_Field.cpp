    ...
    int slot, bool isSetOperation, bool noAccessCheck)
{
    ...
    if (dvmIsStaticField(field)) {
        ...
        if (!dvmVerifyObjectInClass(obj, declaringClass)) {
            assert(dvmCheckException(dvmThreadSelf()));
            if (obj != NULL) {
                ALOGD("Wrong object type for field access: %s is not a %s\n",
    ...
    if (!dvmIsVolatileField(&sfield->field)) {
        ...
        *value = sfield->value;
        ...
        case '[':
            ...
            break;
        default:
            ALOGE("Unhandled field signature '%s'", sfield->field.signature);
            dvmAbort();
        }
    }
}
    ...
    JValue* value)
{
    if (!dvmIsVolatileField(&ifield->field)) {
        ...
        case '[':
            ...
            break;
        default:
            ALOGE("Unhandled field signature '%s'", ifield->field.signature);
            dvmAbort();
        }
    } else {
        /* need memory barriers and/or 64-bit atomic ops */
