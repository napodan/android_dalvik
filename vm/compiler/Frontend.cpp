 ...
 * whole-method traces.
 */
bool dvmCompileMethod(CompilationUnit *cUnit, const Method *method, JitTranslationInfo *info)
{
    const DexCode *dexCode = dvmGetMethodCode(method);
    const u2 *codePtr = dexCode->insns;
    const u2 *codeEnd = dexCode->insns + dexCode->insnsSize;
    ...
    blockList = cUnit->blockList =
        (BasicBlock **)dvmCompilerNew(sizeof(BasicBlock *) * (numBlocks + numInvokeTargets),
                       true);
                     ...
                     int optHints)
{
    ...
    while (1) {
        ...
        if ((flags & kInstrInvoke) &&
            ...
            callsiteInfo->clazz = (const ClassObject*)
                currRun[1].meta;
